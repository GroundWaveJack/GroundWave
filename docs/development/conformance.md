...
Conformance applies to a named specification version and profile. A suite should verify envelope parsing and canonicalization, compact-reference resolution, byte ceilings, identity bindings, policy refusal, delivery dimensions, custody transitions, receipt provenance, adapter mappings, quotas, and downgrade behavior. Rescue conformance additionally distinguishes distress activation, bounded retransmission, responder acceptance, cancellation, marker assignment, relay partition evidence, and stale-state suppression.
